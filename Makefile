...
GEOSPATIAL_COLS := hascoordinate,countrycode,decimallatitude,decimallongitude
...
$(JOINED_FILE): $(VENV_SENTINEL) join.py $(OCC_FILE_ZIP) $(CLUSTERED_STAGE1_FILE)
	mkdir -p $(DATA_DIR)
	$(PYTHON) $(JOIN_SCRIPT) $(OCC_FILE_ZIP) $(CLUSTERED_STAGE1_FILE) $@
join: $(JOINED_FILE)
